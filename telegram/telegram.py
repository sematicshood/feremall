...
from flectra.addons.auth_signup.models.res_partner import random_token
...
Check Help Tab for the rest variables.
    ...
    # bus listener
    @api.model
    def flectra_listener(self, message, bot):
...
class IrConfigParameter(models.Model):
    ...
    def proceed_telegram_configs(self, dbname=False):
        ...
        elif parameter.key == 'telegram.num_flectra_threads':
            message['action'] = 'flectra_threads_changed'
...
class TelegramSession(models.Model):
    ...
    flectra_session_sid = fields.Char(help="Equal to request.session.sid")
    ...
    @api.multi
    def get_flectra_session(self):
        self.ensure_one()
        return flectra.http.root.session_store.get(self.flectra_session_sid)
